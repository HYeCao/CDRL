...
def store_pivots(pos_pivots, neg_pivots, domain):
    ...
    output_dir = "./work/bestpivots/"
    if not os.path.exists(output_dir):
        os.mkdir(output_dir)

    fname = "./work/bestpivots/" + domain +"_pos.txt"
    print(fname)

    with open(fname, "w") as f:
        for i in range(len(pos_pivots)):
            f.write(pos_pivots[i])
            f.write('\n')
    ...
    fname = "./work/bestpivots/" + domain +"_neg.txt"
    print(fname)

    with open(fname, "w") as f:
        for i in range(len(neg_pivots)):
            f.write(neg_pivots[i])
            f.write('\n')
